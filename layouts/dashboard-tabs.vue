			...
			>
				...
				<div class="flex items-center gap-4 sm:hidden">
					<Select
						v-model="selectedUid"
						@update:model-value="
							handleTabTrigger(selectedTab?.href ?? '#')
						"
					>
						<SelectTrigger
							class="text-md h-6 w-[124px] rounded-md border px-3 py-1.5"
						>
				...
				<section
					class="hidden flex-col items-start justify-between space-y-4 sm:flex md:flex-row md:items-center md:space-y-0"
				>
					<TabsList>
						<TabsTrigger
							v-for="tab in tabs"
...
const { currentPageInfo, loadingPage } = storeToRefs(useAppStateStore())
